 ...
 * User: jgreco
 */
public class FIXOrderEntryTest extends GenericAppTest<FixOrder> {
	private FixOrderEntry app;
	private FixStore store;
	private FixWriter writer;
	private final FixStubTags tags = new FixStubTags();

	public FIXOrderEntryTest()
	{
	...
	public void testRejectOnMarketNotOpen() {
		...
		verify(writer).writeString(tags.Symbol, str("10Y"));
	...
	private void setInitialReqFields(String clOrdId) { 
		...
		command.setCUSIP(ByteBuffer.wrap("CUSIP_10Y".getBytes()));

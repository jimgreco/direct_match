 ...
 * User: jgreco
 */
public class FixOrderEntryForTT extends MatchApplication
		implements
    ...
    private final FixOrderRepository clOrdIdStore;
	private final FixOrderService orderService;

	private final FIXQtyMode qtyMode;
    private final FIXOrderOutputForTT<FixOrder> fixOutput;
	...
	@AppConstructor
    public FixOrderEntryForTT(Log log,
		...
		this(log, dispatcher, timeFactory, traderService, accountService, securityService, systemEventService, contributorService, sender, connector, msgs, new FixServerTcpConnector(log, socketFactory, sender, port),
                new InlineFixParser(), new InlineFixWriter(timeSource, version, targetCompID, senderCompID), new FixFileStore(
                        fileFactory, log, sender.getName()), new FixDispatcher(log), senderCompID, targetCompID, accountName, FIXQtyMode.RoundLot, version);

		this.info = new FIXPortInfo(port, version, senderCompID, targetCompID);
	}
	...
	public FixOrderEntryForTT(Log log,
		...
		this.messages = msgs;
		this.stateMachine = new FixStateMachine(log, fixDispatcher, timerService, sender, parser, store, fixConnector, msgs, false, false, fixVersion, senderCompID, targetCompID);
		...
		this.clOrdIdStore = new FixOrderRepository(CONCURRENT_ORDERS);
		this.accountName = accountName;
		this.orderService.addListener(clOrdIdStore);
		this.orderService.addRejectListener(clOrdIdStore);
		this.orderService.addListener(this);
		this.orderService.addRejectListener(this);
		contributorService.addListener((contributor1, msg, isNew) -> FixOrderEntryForTT.this.contributor = contributor1);
        ...
        this.fixOutput = new FIXOrderOutputForTT<>(parser, store, this.qtyMode, traderService, accountService, securityService);
	...
	@Override
        ...
        if (clOrdIdStore.seenClOrdID(clOrdId.getValue())) {
	...
	public void onFixOrderCancelRequest() {
		...
		FixOrder order = clOrdIdStore.getOrder(origClOrdId.getValue());
		if (order == null) {
			boolean everyExisted = clOrdIdStore.seenClOrdID(origClOrdId.getValue());
			if (everyExisted) {
